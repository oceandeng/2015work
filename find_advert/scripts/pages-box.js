...
! function(t, s) {
    function i(s, i) {
        return this.options = t.extend({}, r, i),
                this.ul = t(s),
                ...
                ? 
                    !1 
                : 
                    (this.options.scrollPage && (this.scrollPage = this.ul.find(this.options.scrollPage),
                    this.scrollIndex = this.scrollPage.index()),
                    this.options.unTouchedPages && (this.unTouchedPages = this.ul.find(this.options.unTouchedPages)),
                    this._initPx(),
                    this.page = 0,
                    this.prePage = 0,
                    this.started = !1,
    ...
    i.prototype._initPx = function() {
        this.unitHeight = "y" === this.axis ? this.pages.eq(0).height() : this.pages.eq(0).width(),
        this.scrollHeight = this.options.scrollPage ? "y" === this.axis ? this.scrollPage.height() : this.scrollPage.width() : 0,
        this.height = "y" === this.axis ? this.unitHeight * (this.length - 1) + this.scrollHeight : this.unitHeight * (this.length) + this.scrollHeight,
    ...
    i.prototype._initPageArr = function() {
        var s, i, h = 0,
            o = 0,
            e = this;
        for (this.pageArr = [], i = 0; i < this.pages.length; i++) {
            o = "y" === this.axis ? this.pages.eq(i).height() : this.pages.eq(i).width(),
            this.pageArr.push([-h, -h - o + 1]),
            h += o;
        }

        this.scrollTransArr = [],
        this.options.scrollPage && (this.scrollTransArr = [this.pageArr[this.scrollIndex][0], this.pageArr[this.scrollIndex][1] + this.unitHeight]),
        this.unTouchedArr = [],
        this.options.unTouchedPages && this.unTouchedPages.each(function() {
    ...
    i.prototype.init = function() {
        ...
        t('.arrow').on('tap', function(e){
            i.goNext.call(i, e);
        });

        this.ul[0].addEventListener("webkitTransitionEnd", function(t) {
            i._transitionend.call(i, t),
            i.ul.trigger("transend", i.page)
        }, !1),
        t(s).on("orientationchange", function() {
            var t = i._getTrans()[this.axis],
                s = t - i.pageArr[i.page][0];

            i._initPx(),
            i._initPageArr(),
            i.goStep(s + i.pageArr[i.page][0])
        });
    ...
    i.prototype._touchstart = function(t) {
        ...
        return this.started = !1,
                ...
                this.options.unTouchedPages && this.unTouchedArr.indexOf(this.startTrans[this.axis]) > -1 ? !1 : (this.started = !0,
                this.moved = !1,
                void(this.startTime = t.timeStamp))
    },
    ...
    i.prototype._touchend = function(t) {
        var s, i, h = t.changedTouches[0],
            o = 0,
            e = 0;
        this.options.scrollPage && (o = this.scrollTransArr[0],
        e = this.scrollTransArr[1]), this.endPos = {
            x: h.pageX,
            y: h.pageY
        },
        ...
        if (this.trans = this._getTrans(), this.endTime = t.timeStamp, this.endPos[this.axis] < this.startPos[this.axis]) {
            if (this.options.scrollPage){
                if (this.options.fromScrollToOther || this.startTrans[this.axis] !== e){
                    if (this.scrollIndex === this.length - 1 && this.startTrans[this.axis] === e) {
                        ...
                        if (this.startTrans[this.axis] > o || this.startTrans[this.axis] < e) {
                            return void(Math.abs(this.endPos[this.axis] - this.startPos[this.axis]) >= this.options.threshold ? this.goNext() : this.goPresent());
                        }
                ...
                Math.abs(this.endPos[this.axis] - this.startPos[this.axis]) >= this.options.threshold ? this.goNext() : this.goPresent();
            }
        } else if (this.options.scrollPage){
            if (this.options.fromScrollToOther || this.startTrans[this.axis] !== o) {
                if (this.scrollIndex < this.length - 1 && this.startTrans[this.axis] === this.pageArr[this.scrollIndex + 1][0]) {
                    this.goPage(this.scrollIndex, this.page);
                }else {
                    if (this.startTrans[this.axis] > o || this.startTrans[this.axis] < e) {
                        return void(Math.abs(this.endPos[this.axis] - this.startPos[this.axis]) >= this.options.threshold ? this.goPrev() : this.goPresent());
                    }
            ...
            Math.abs(this.endPos[this.axis] - this.startPos[this.axis]) >= this.options.threshold ? this.goPrev() : this.goPresent()
        }
    },
    ...
    i.prototype._getTrans = function() {
        var t,
            s = this.ul[0].style.webkitTransform || "",
    ...
    i.prototype._addDots = function(){
        var dots = '<div class="dotsBox">';
        for(var i = 0; i < this.pages.length; i ++){
            dots += '<span class="dots" id = "' + i + '"></span>';
        }
        dots += '</div>';
        $(this.ul).parent().append(dots);
        $('.dotsBox').find('span').eq(0).addClass('active');
    },
    i.prototype.getAxis = function() {
        return this.options.axis
    },
    i.prototype.goStep = function(t) {
        this.trans[this.axis] = t,
    ...
    i.prototype.goPage = function(t, s) {
        ...
        this.addTrans();
        var i = 0;
        t >= this.pageArr.length ? t = s : 0 > t && (t = 0),
        i = this.pageArr[t][0],
        this._setTransform(i),
        this.animating = !0,
        this.ul.trigger("gopage", [t, s])
    },
    ...
    t.fn[h] = function(s) {
        if ("string" == typeof s) {
            ...
            if (Array.prototype.shift.call(o), "check" == r) {
                return !!this.data("plugin_" + h);
            }
            if (a()) {
