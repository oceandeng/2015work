    ...
    classes.Scroller = function (el, settings, inherit) {
        ...
        function generateWheelItems(i) {
            ...
            $.each(values, function (j, v) {
                ...
                html += '<div role="option" aria-selected="false" class="dw-li dw-v" data-val="' + keys[j] + '"' + (labels ? ' aria-label="' + labels + '"' : '') + ' style="height:' + itemHeight + 'px;line-height:' + itemHeight + 'px;">' +
                    '<div class="dw-i"' + (lines > 1 ? ' style="line-height:' + Math.round(itemHeight / lines) + 'px;font-size:' + Math.round(itemHeight / lines * 0.8) + 'px;"' : '') + '>' + v + labels + '</div></div>';
                l++;
            });
