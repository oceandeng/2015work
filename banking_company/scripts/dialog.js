...
(function(G, D) {
    ...
    Dialog.prototype.create = function() {
        ...
        switch(this.config_type){
            case "input":
                var html = "";
                html += "<div class='D_tit' style='color:#19273f'>建议授信额度</div>";
    ...
    Dialog.prototype.validate =function(){
        var thisVal = $(this.input).val() * 1;
        var timer = null;

        $(this.input).on('keyup', function(){
            var $__this = $(this);
            var modifyVal = $__this.val() * 1;

            clearTimeout(timer);
            timer = setTimeout(function(){
                valiFn(modifyVal, thisVal, $__this);
            }, 1000);

            $__this.val(modifyVal);

            console.log(typeof modifyVal);

        });

        function valiFn(modifyVal, thisVal, context){
            if(isNaN(modifyVal)){
                oTools.alertmess('请输入数字!');
                context.val(thisVal);
                return;
            }
            if(!modifyVal){
            ...
            if(modifyVal < 10000 ){
                oTools.alertmess('金额不能小于10,000!');
                return;
            }
            if(modifyVal > 50000){
                oTools.alertmess('金额不能大于50,000!');
                return;
            }
        }
