...
$(function(){
	...
	function topBg(){
		...
		$topHeader.css({
			height: winH - $('header').height()
		});

		topHeaderCH = $topHeader.height();
		...
		var cH = $('.top-header-content').height();

		$('.top-header-content').css({
			paddingTop: (topHeaderCH - cH + 60) / 2
		})
	}
...
$(window).load(function(){
	...
	$('#mainWrap').load('include/content.html', function(){
		...
		$('img').load(function(){
			...
			// 	loadJS('scripts/dakaweb/scrollreveal.js');
			// 	num ++;
			// }
			...
			$('.p-s-con').find('a').on('mouseenter', function(){
				...
				if(/and/.exec(iSrc)){
					$img.attr('src', gConfig.imagePath + 'images/dakaweb/and-icon-a.png');
				}else if(/ios/.exec(iSrc)){
					$img.attr('src', gConfig.imagePath + 'images/dakaweb/ios-icon-a.png');
				}else{
			...
			$('.p-s-con').find('a').on('mouseleave', function(){
				...
				if(/and/.exec(iSrc)){
					$img.attr('src', gConfig.imagePath + 'images/dakaweb/and-icon-n.png');
				}else if(/ios/.exec(iSrc)){
					$img.attr('src', gConfig.imagePath + 'images/dakaweb/ios-icon-n.png');
				}else{
